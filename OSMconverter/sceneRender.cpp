...
GLuint sceneRender::tessellateRoof(vector<Building> *buildingList)
{
	...
	for (int i = 0; i < (*buildingList).size(); i++)
	{
			...
			{				
				...
				//vertex[ind][1] = (*buildingList)[i].nodes[j].height + (*buildingList)[i].BuildingHeight;
	...
	for (int i = 0; i < (*wayList).size(); i++)
	{
		...
		case wayType::amenityMotorCycleParking:
			...
			for (int j = 0; j < (*wayList)[i].nodes.size(); j++)
			{
				...
				vertex[ind][1] = scene.terrainLoader.getTerrainHeight((*wayList)[i].nodes[j].lat, (*wayList)[i].nodes[j].lon);
...
void sceneRender::drawTerrainTriangle()
{
	GLfloat color;
	glPushMatrix();
	glTranslatef(scene.terrain.shiftx, 0, scene.terrain.shiftz);
...
void sceneRender::initScene(void)
{
	...
	//glEnable(GL_CULL_FACE); //normalleri koyunca duzgun calisicak
	...
	mot.cameraPosition.Change(-5.0, 3.0, 4.0);
	mot.lookatPosition.Change(-5.0, 3.0, -6.0);
	...
	//roadTestTexture = bmploader.LoadGLTexturesBMP("Textures/roadtest.bmp");
...
void sceneRender::drawHighways()
{
	for (int i = 0; i < scene.parser.highWayList.size(); i++)
	{
		...
		case wayType::railway:
			glColor3f(1.0f, 1.0f, 1.0f);
			Draw3DWayNew(&scene.parser.highWayList[i], railwayTexture, 0.05);
			continue;

		case wayType::highwayResidual:
			glColor3f(0.8f, 0.8f, 0.8f);
			Draw3DWayNew(&scene.parser.highWayList[i], roadResidualTexture, 0.01);
			continue;

		case wayType::highwayUnclassified:
			glColor3f(0.8f, 0.8f, 0.8f);
			Draw3DWayNew(&scene.parser.highWayList[i], roadUnclassifiedTexture, 0.015);
			continue;

		case wayType::highwayService:
			glColor3f(1.0f, 1.0f, 1.0f);
			Draw3DWayNew(&scene.parser.highWayList[i], roadServiceTexture, 0.009);
			continue;

		case wayType::highwayPrimary:
			glColor3f(1.0f, 1.0f, 1.0f);
			Draw3DWayNew(&scene.parser.highWayList[i], roadPrimaryTexture, 0.03);
			continue;

		case wayType::highwaySecondary:
			glColor3f(1.0f, 1.0f, 1.0f);
			Draw3DWayNew(&scene.parser.highWayList[i], roadSecondaryTexture, 0.025);
			continue;

		case wayType::highwayTertiary:
			glColor3f(0.8f, 0.8f, 0.8f);
			Draw3DWayNew(&scene.parser.highWayList[i], roadTertiaryTexture, 0.035);
			continue;

		case wayType::highwayPedestrian:
			glColor3f(0.9f, 0.8f, 0.8f);
			Draw3DWayNew(&scene.parser.highWayList[i], roadPavementTexture, 0.005);
			continue;
		case wayType::highwayPath:
			continue;
			glColor3f(1.0f, 1.0f, 1.0f);
			Draw3DWayNew(&scene.parser.highWayList[i], roadPathTexture, 0.005);
			continue;

		case wayType::river:
			glColor3f(0.7f, 0.7f, 0.7f);
			Draw3DWayNew(&scene.parser.highWayList[i], riverTexture, 0.005);
			continue;
		}
	}
...
void sceneRender::Draw3DWay(HighWay *way, GLuint texture, float layerheight)
{
	...
	for (int j = 0, k = 0; j < (*way).nodes3DSize-4; j += 4, k++)
	{
		glTexCoord2f(0.0f, roadpos);
		glVertex3f((*way).nodes3D[j], layerheight + scene.terrainLoader.getTerrainHeight((*way).nodes[k].lat, (*way).nodes[k].lon), (*way).nodes3D[j + 1]);
		glTexCoord2f(1.0f, roadpos);
		glVertex3f((*way).nodes3D[j + 2], layerheight + scene.terrainLoader.getTerrainHeight((*way).nodes[k].lat, (*way).nodes[k].lon), (*way).nodes3D[j + 3]);
		int a1 = (*way).nodes[k + 1].meterx - (*way).nodes[k].meterx;
		int a2 = (*way).nodes[k + 1].meterz - (*way).nodes[k].meterz;
		roadpos += (sqrt(a1*a1 + a2*a2) / 5.0f);
		glTexCoord2f(1.0f, roadpos);
		glVertex3f((*way).nodes3D[j + 6], layerheight + scene.terrainLoader.getTerrainHeight((*way).nodes[k + 1].lat, (*way).nodes[k + 1].lon), (*way).nodes3D[j + 7]);
		glTexCoord2f(0.0f, roadpos);
		glVertex3f((*way).nodes3D[j + 4], layerheight + scene.terrainLoader.getTerrainHeight((*way).nodes[k + 1].lat, (*way).nodes[k + 1].lon), (*way).nodes3D[j + 5]);
	}
	glEnd();
	glDisable(GL_TEXTURE_2D);
}

void sceneRender::Draw3DWayNew(HighWay *way, GLuint texture,float layerheight)
{
	...
	for (int i = 0; i < min((int)way->leftSideVertexes.size(), (int)way->rightSideVertexes.size())- 1; i++)
	{
		...
		int a1 = way->leftSideVertexes[i+1].x - way->leftSideVertexes[i].x;
		int a2 = way->leftSideVertexes[i + 1].z - way->leftSideVertexes[i + 1].z;
		roadpos += (sqrt(a1*a1 + a2*a2) / 5.0f);
		glTexCoord2f(1.0f, roadpos);
